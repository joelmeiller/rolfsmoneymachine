...
async function startServer() {
    ...
    app.use(morgan('[:date[web]] :nmethod :status :url - :response-time ms', {
    ...
    // Config
    // Health check
    app.get('/health/ready', (_req, res) => res.send('healthy') // used by Kubernetes to decide when the application is ready to receive traffic
    );
    ...
    console.log(logStyleStr, ` 🌸 🧚🏻‍♀️ 🌺 Backend for Frontend available (BfF) at port ${Config.PORT} under ${Config.PUBLIC_URL} 🌼 🐉 🌻 `);
    console.log('');
}
